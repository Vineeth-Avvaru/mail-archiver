const Mails = [
    {
        from: "aaa@example.com",
        to: ["bbb@example.com"],
        subject: "Lottery Time !! ",
        date: "06/12/2020 10:50",
        body: "Only 1 out of 50k won the lotter and it's you!! Sending you the document to fill your bank account info.",
        attachment: true
    },
    {
        from: "jingping@example.com",
        to: ["ggg@example.com","ccc@example.com","ddd@example.com"],
        subject: "Travel Insurance ",
        date: "06/05/2020 17:50",
        body: "Taking Travel Insurance is always good for health and your loved ones",
        attachment: false
    },
    {
        from: "jonsmith@example.com",
        to: ["babusmilz@example.com"],
        subject: "Fun time at the circus",
        date: "06/02/2020 19:50",
        body: "Come and Have a good time at the new tokyo circus coming near you. see the attachment for details",
        attachment: true
    },
    {
        from: "careworld@example.com",
        to: ["grandma@example.com","ccc@example.com","ddd@example.com"],
        subject: "Time to retire",
        date: "05/26/2020 13:14",
        body: "Welcome to careworld!! The perfect paradise for retirement",
        attachment: false
    },
    {
        from: "chaku@example.com",
        to: ["samaram@example.com"],
        subject: "This is to inform you guys that you are winner for the lucky draw",
        date: "05/12/2020 10:45",
        body: "Hurrayyy!! It's lucky time!!",
        attachment: false
    },
    {
        from: "bali@example.com",
        to: ["platiz@example.com","ccc@example.com","ddd@example.com"],
        subject: "Credit Card Offers",
        date: "05/10/2020 01:45",
        body: "Very Less time to apply of the new offers!",
        attachment: true
    }
];

export default Mails;